...
class TestInformationGain(unittest.TestCase):
    ...
    def test_perfect_binary_split(self):
        ...
        data = np.array([[1.0, 5.0, 0], [1.0, 6.0, 0], [2.0, 5.0, 1], [2.0, 6.0, 1]])
    ...
    def test_perfect_split_three_classes(self):
        ...
        data = np.array([[1.0, 5.0, 0], [1.0, 6.0, 0], [2.0, 5.0, 1], [2.0, 6.0, 1], [3.0, 5.0, 2], [3.0, 6.0, 2]])
        result = gain(data, feature_index=0, threshold=1.5)
    ...
    def test_no_gain_useless_split(self):
        ...
        data = np.array([[1.0, 5.0, 0], [1.0, 6.0, 1], [2.0, 5.0, 0], [2.0, 6.0, 1]])
        result = gain(data, feature_index=0, threshold=1.5)
        # Both subsets have same 50-50 distribution as original
        # Gain should be approximately 0
        self.assertAlmostEqualGain(result, 0.0, places=5, msg="Useless split should have gain close to 0")

    def test_no_gain_pure_dataset(self):
        """Splitting an already pure dataset should give gain = 0."""
        data = np.array([[1.0, 5.0, 0], [1.5, 6.0, 0], [2.0, 7.0, 0], [2.5, 8.0, 0]])
    ...
    def test_partial_improvement_split(self):
        ...
        data = np.array([[1.0, 5.0, 0], [1.2, 5.5, 0], [1.4, 6.0, 0], [2.0, 5.0, 1]])
        result = gain(data, feature_index=0, threshold=1.5)
        ...
        self.assertAlmostEqualGain(result, expected_gain, places=5, msg="Partial improvement split gain mismatch")
    ...
    def test_varying_split_quality(self):
        ...
        data = np.array([[1.0, 10.0, 0], [1.0, 20.0, 0], [2.0, 10.0, 1], [2.0, 20.0, 1]])
        ...
        self.assertGreater(gain_perfect, gain_useless, msg="Perfect split should have higher gain than useless split")
    ...
    def test_single_sample_left(self):
        ...
        data = np.array([[1.0, 5.0, 0], [2.0, 6.0, 0], [2.5, 7.0, 1], [3.0, 8.0, 1]])
        result = gain(data, feature_index=0, threshold=1.5)
        ...
        self.assertAlmostEqualGain(result, expected_gain, msg="Single sample split gain mismatch")
    ...
    def test_threshold_at_boundary(self):
        ...
        data = np.array([[1.0, 5.0, 0], [2.0, 6.0, 0], [2.0, 7.0, 1],  # Note: same value as previous row
            [3.0, 8.0, 1]])
    ...
    def test_different_features(self):
        ...
        self.assertGreater(gain_feature_0, 0.9, msg="Perfect split should have high gain")
        self.assertLess(gain_feature_1, 0.1, msg="Poor split should have low gain")

    # ========== Multi-class Tests ==========

    def test_three_class_split(self):
        """Test gain calculation with three classes."""
        data = np.array([[1.0, 5.0, 0], [1.5, 5.5, 0], [2.0, 6.0, 1], [2.5, 6.5, 1], [3.0, 7.0, 2], [3.5, 7.5, 2]])
        ...
        expected_gain = initial_entropy - (
                    len(left_data) / len(data) * left_entropy + len(right_data) / len(data) * right_entropy)

        self.assertAlmostEqualGain(result, expected_gain, msg="Three-class split gain mismatch")

    # ========== Property Tests ==========

    def test_gain_non_negative(self):
        """Information gain should always be non-negative."""
        test_cases = [(np.array([[i, i, i % 2] for i in range(10)]), 0, 5.0),
        ...
        for data, feature_idx, threshold in test_cases:
            ...
            self.assertGreaterEqual(result, 0.0, msg="Information gain should be non-negative")
    ...
    def test_large_dataset_gain(self):
        ...
        self.assertGreater(result, 0.1, msg="Correlated feature should provide positive gain")

    # ========== Numerical Stability Tests ==========

    def test_no_nan_or_inf(self):
        """Ensure gain calculation doesn't produce NaN or Inf."""
        test_cases = [np.array([[1.0, 2.0, 0], [1.0, 2.0, 0]]),  # All same
